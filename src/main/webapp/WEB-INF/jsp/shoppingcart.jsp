...
$("#delbtn").click(function(){
	var groupCheckbox=$("input[name='mycheck']");
for(i=0;i<groupCheckbox.length;i++){
  if(groupCheckbox[i].checked){
      var val =groupCheckbox[i].value;
      var username = ${sessionScope.username}
      //删除
      location.href = "/delbtn?goodscarid="+val+"&username="+username; 
  }
}	
})
...
$("#toordermsg").click(function(){
  ...
  //减号
  function minusbtn(goodscarid,username,goodscount){
	  if(goodscount == 1){
		  if(confirm("确定要删除吗？")){
			 location.href = "/minuschange?goodscarid="+goodscarid+"&goodscount="+goodscount+"&username="+username; 
		  }
	  }else{
		  location.href = "/minuschange1?goodscarid="+goodscarid+"&goodscount="+goodscount+"&username="+username; 
	  }
  }
 //加号
 function plusbtn(goodscarid,username,goodscount){
	 location.href = "/pluschange?goodscarid="+goodscarid+"&goodscount="+goodscount+"&username="+username; 
 }
//安全结账按钮
function savebtn(username){
	var aaa = $('input:radio:checked').val();
	if(aaa == null||aaa == ""||aaa == undefined){
		alert("请选择订单！")
	}else{
	 if(confirm("确定付款吗？")){
		 //获取选中的goodscarid
		 location.href = "/showorder?username="+username+"&goodscarid="+aaa;
 	}else{
	...
	<div class="row clearfix">
		<div class="col-md-12 column">
			<table border="2" style="width: 100%;">
				...
				<tr>
				<th>
				<div style="text-align: center"><input type="radio" class="mycheck" name="mycheck" value="${list.goodscarid}"  val="${list.totalprice}" style="height: 40px;width: 40px;border:none;"/></div>
				</th>
					...
					<th style="text-align:center;">
					<input type="button" id="minusbtn" class="changebtn" onclick="minusbtn(${list.goodscarid},'${sessionScope.username}',${list.goodscount})" style="height: 40px;width: 30px;" value="-" />&nbsp;&nbsp;&nbsp;
                    <span>${list.goodscount}</span>&nbsp;&nbsp;&nbsp;
    				<input type="button" id="plusbtn" class="changebtn" onclick="plusbtn(${list.goodscarid},'${sessionScope.username}',${list.goodscount})" style="height: 40px;width: 30px;" value="+" />
					</th>
	...
	<div class="row clearfix">
		...
		<div class="col-md-4 column">
			<br>
			<button style="color: white;background-color: black;border:none;width: 150px;height: 45px;" id="savebtn" onclick="savebtn('${sessionScope.username}')"><h4>安全结账</h4></button></div>
		</div>
	</div>
		<br /><br />	 
